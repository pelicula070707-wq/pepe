...
app.post('/api/reservar', async (req, res) => {
  ...
  if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
    console.log('⚠️  Modo demo — reserva:', { nombre, email, servicio, fecha, hora });
    return res.json({ success: true, message: `¡Reserva registrada, ${nombre}! (Modo demo: configura SMTP para enviar correos reales)` });
  }
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      html: `<h2>Nueva Reserva</h2><p><b>Cliente:</b> ${nombre}</p><p><b>Email:</b> ${email}</p><p><b>Teléfono:</b> ${telefono}</p><p><b>Servicio:</b> ${servicio}</p><p><b>Fecha:</b> ${fecha}</p><p><b>Hora:</b> ${hora}</p><p><b>Comentarios:</b> ${comentarios || 'Ninguno'}</p>`
    });
    console.log(`✅ Reserva: ${nombre} — ${fecha} ${hora}`);
    res.json({ success: true, message: `¡Reserva confirmada, ${nombre}! Te enviamos un correo a ${email}.` });
  } catch (error) {
    console.error('❌ Error SMTP:', error.message);
    res.status(500).json({ success: false, message: 'Reserva registrada, pero hubo un error al enviar el correo. Contáctanos directamente.' });
  }
});
